...
class TfidfBaseline:
    ...
    def build_corpus(self, comments, score_thresh):
        corpus = []
        labels = []
        for comment in comments:
    ...
    def build_all_corpus(self):
        ...
        # baseball
        print('baseball:')
        self.baseball_train_corpus, self.baseball_train_labels = self.build_corpus(self.baseball_train_comments, self.baseball_score_thresh)
        self.baseball_test_corpus, self.baseball_test_labels = self.build_corpus(self.baseball_test_comments, self.baseball_score_thresh)

        # movie
        print('movie:')
        self.movie_train_corpus, self.movie_train_labels = self.build_corpus(self.movie_train_comments, self.movie_score_thresh)
        self.movie_test_corpus, self.movie_test_labels = self.build_corpus(self.movie_test_comments, self.movie_score_thresh)

        # lol
        print('lol:')
        self.lol_train_corpus, self.lol_train_labels = self.build_corpus(self.lol_train_comments, self.lol_score_thresh)
        self.lol_test_corpus, self.lol_test_labels = self.build_corpus(self.lol_test_comments, self.lol_score_thresh)
    ...
    def train(self, train_corpus, train_labels, type='bayes', nu=0.5, gamma=0.001):
        if type == 'bayes':
            ...
            classifier = Pipeline([('vect', CountVectorizer()),
                           ...
                           #('clf', svm.SVC())
                           ('clf', svm.NuSVC(nu=nu, gamma=gamma))
                           #('clf', svm.LinearSVC())
            ])
            _ = classifier.fit(train_corpus, train_labels)
